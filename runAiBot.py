...
# Import new module for handling questions
from modules.question_handler import handle_categorical_question, handle_experience_question, handle_personal_info_question
    ...
    for Question in all_questions:
        ...
        if select:
            ...
            if overwrite_previous_answers or selected_option == "Select an option":
                ...
                # Add location handling
                    ...
                    # Try explicit mapping from categorical_answers first!
                    mapped = handle_categorical_question(label_org, optionsText, categorical_answers)
                    if mapped:
                        answer = mapped
                    else:
                        # Try experience mapping
                        exp_mapped = handle_experience_question(label_org, optionsText, skill_experience, years_of_experience, min_experience_for_unknown_skills)
                        if exp_mapped:
                            answer = exp_mapped
                        else:
                    ...
                    for phrase in possible_answer_phrases:
                        for option in optionsText:
                            ...
                            if phrase.lower() in option.lower() or option.lower() in phrase.lower():
                                ...
                                break
                    if not foundOption:
                        # Try AI to answer the question if enabled
                        ai_answered = False
                        if use_AI and aiClient:
                            try:
                                ai_result = ""
                                if ai_provider.lower() == "openai":
                                    ai_result = ai_answer_question(aiClient, label_org, options=optionsText, question_type="select", job_description=job_description, user_information_all=user_information_all)
                                elif ai_provider.lower() == "deepseek":
                                    ai_result = deepseek_answer_question(aiClient, label_org, options=optionsText, question_type="select", job_description=job_description, about_company=None, user_information_all=user_information_all)
                                elif ai_provider.lower() == "gemini":
                                    ai_result = gemini_answer_question(aiClient, label_org, options=optionsText, question_type="select", job_description=job_description, about_company=None, user_information_all=user_information_all)
                                
                                if ai_result and ai_result in optionsText:
                                    select.select_by_visible_text(ai_result)
                                    answer = ai_result
                                    ai_answered = True
                                    print_lg(f'AI answered question labelled "{label_org}" with "{answer}"')
                            except Exception as e:
                                print_lg("Failed to get AI answer!", e)
                        
                        if not ai_answered:
                            if pause_at_failed_question and not run_in_background:
                                pyautogui.alert(f'Bot failed to answer dropdown: "{label_org}". Options: {optionsText}. Please answer it manually then click OK to continue.', "Manual Input Needed")
                                answer = select.first_selected_option.text
                            else:
                                print_lg(f'Failed to find an answer for question labelled "{label_org}" and pause_at_failed_question is false! Leaving blank or keeping default.')
        ...
        if radio:
            ...
            if overwrite_previous_answers or prev_answer is None:
                # Try explicit mapping from categorical_answers first!
                mapped = handle_categorical_question(label_org, options_labels, categorical_answers)
                if mapped:
                    answer = mapped
                elif 'veteran' in label or 'protected' in label: answer = veteran_status
                elif 'disability' in label or 'handicapped' in label: answer = disability_status
                else: 
                    exp_mapped = handle_experience_question(label_org, options_labels, skill_experience, years_of_experience, min_experience_for_unknown_skills)
                    if exp_mapped: 
                        answer = exp_mapped
                    else:
                        answer = answer_common_questions(label,answer)
                ...
                if foundOption: 
                    ...
                    if not foundOption: 
                        if pause_at_failed_question and not run_in_background:
                            pyautogui.alert(f'Bot failed to answer radio group: "{label_org}". Options: {options_labels}. Please answer it manually then click OK to continue.', "Manual Input Needed")
                        else:
                            print_lg(f'Failed to find answer for radio "{label_org}". It may be wrong or blank.')
        ...
        if text: 
            ...
            if not prev_answer or overwrite_previous_answers:
                personal_info = {
                    ...
                    "street": street,
                    "current_city": current_city if current_city else work_location,
                    "full_name": full_name,
                ...
                # Try mappings
                personal_mapped = handle_personal_info_question(label_org, personal_info)
                exp_mapped = handle_experience_question(label_org, [], skill_experience, years_of_experience, min_experience_for_unknown_skills)
                
                if personal_mapped:
                    answer = personal_mapped
...
def submitted_jobs(job_id: str, title: str, company: str, work_location: str, work_style: str, description: str, experience_required: int | Literal['Unknown', 'Error in extraction'], 
                   ...
                   questions_list: set | None, connect_request: Literal['In Development'], profile_match: str | Literal['Unknown'] = 'Unknown', salary_extracted: str | Literal['Unknown'] = 'Unknown') -> None:
    '''
    Function to create or update the Applied jobs CSV file, once the application is submitted successfully.
    Includes advanced tracking features like Profile Match (%) and JD Extracted Salary.
    '''
    try:
        with open(file_name, mode='a', newline='', encoding='utf-8') as csv_file:
            fieldnames = ['Job ID', 'Title', 'Company', 'Work Location', 'Work Style', 'About Job', 'Experience required', 'Skills required', 'Profile Match (%)', 'Salary Extracted', 'HR Name', 'HR Link', 'Resume', 'Re-posted', 'Date Posted', 'Date Applied', 'Job Link', 'External Job link', 'Questions Found', 'Connect Request']
            writer = csv.DictWriter(csv_file, fieldnames=fieldnames)
            if csv_file.tell() == 0: writer.writeheader()
            writer.writerow({'Job ID':truncate_for_csv(job_id), 'Title':truncate_for_csv(title), 'Company':truncate_for_csv(company), 'Work Location': "", 'Work Style': "", 
                            'About Job': "", 'Experience required': truncate_for_csv(experience_required), 'Skills required': "", 
                                'Profile Match (%)': truncate_for_csv(profile_match), 'Salary Extracted': truncate_for_csv(salary_extracted),
                ...
                for job in job_listings:
                    ...
                    if use_AI and description != "Unknown":
                        ...
                        try:
                            ...
                            # Note: The AI modules (openai_integration, etc) need to be updated to support
                            # extracting profile_match and salary. For now, we stub these out.
                            # We can assume the JSON returned will eventually have these keys.
                            # Try parsing if it's returning a larger JSON, else defaults remain.
                        except Exception as e:
                    ...
                    if try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3') and contains(@aria-label, 'Easy')]"):
                        ...
                        if dailyEasyApplyLimitReached:
                            ...
                            return
                        if skip: continue
                    submitted_jobs(job_id, title, company, work_location, work_style, description, experience_required, skills, hr_name, hr_link, resume, reposted, date_listed, date_applied, job_link, application_link, questions_list, connect_request, profile_match, salary_extracted)
