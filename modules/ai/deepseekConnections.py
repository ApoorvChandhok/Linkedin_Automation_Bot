...
def deepseek_extract_skills(client: OpenAI, job_description: str, stream: bool = stream_output) -> tuple[list[str], str, str]:
    '''
    Extracts skills, profile match, and salary from job description using DeepSeek API.
    * Takes in `client` of type `OpenAI` - The DeepSeek client
    ...
    * Returns a tuple (skills_list, profile_match string, salary_extracted string).
    '''
    try:
        ...
        skills = result.get("required_skills", []) if isinstance(result, dict) else []
        profile_match = result.get("profile_match", "Unknown") if isinstance(result, dict) else "Unknown"
        salary_extracted = result.get("salary_extracted", "Unknown") if isinstance(result, dict) else "Unknown"
        
        return skills, profile_match, salary_extracted
    except Exception as e:
