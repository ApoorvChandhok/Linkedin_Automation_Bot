...
def ai_extract_skills(client: OpenAI, job_description: str) -> tuple[list[str], str, str]:
    """
    Extracts required skills, profile match, and salary from the given job description using OpenAI.
    ...
    Returns:
        tuple: (A list of required skills, profile_match string, salary_extracted string).
    """
    print_lg("-- EXTRACTING SKILLS FROM JOB DESCRIPTION")
    messages = [
        {"role": "user", "content": extract_skills_prompt.format(job_description)}
    ]

    try:
        response = client.chat.completions.create(
            model=llm_model, # Changed from ai_model to llm_model for consistency
            messages=messages,
        ...
        if not content:
            print_lg("Warning: Received empty content from OpenAI.")
            return [], "Unknown", "Unknown"
        
        # Now parse the JSON content
        import json
        extracted_data = json.loads(content)
        skills_matched = extracted_data.get("required_skills", [])
        profile_match = extracted_data.get("profile_match", "Unknown")
        salary_extracted = extracted_data.get("salary_extracted", "Unknown")
        
        return skills_matched, profile_match, salary_extracted
