...
def gemini_extract_skills(model, job_description: str) -> tuple[list[str], str, str]:
    """
    Extracts skills, profile match, and salary from a job description using the Gemini model.
    * Takes in `model` - The Gemini model object.
    ...
    * Returns a tuple (skills_list, profile_match string, salary_extracted string).
    ...
    try:
        ...
        response_data = gemini_completion(model, prompt, is_json=True)
        
        if isinstance(response_data, dict):
            skills = response_data.get("required_skills", [])
            profile_match = response_data.get("profile_match", "Unknown")
            salary_extracted = response_data.get("salary_extracted", "Unknown")
